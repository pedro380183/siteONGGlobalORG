...
import { img1, img2, img3, img4, img5, img6, img7, img8, img9, img10, img11, img12 } from './Imagens'
...
const GerarRedesSociais =(props)=> {
  const Data = props.arr.map(item => {
    return <li><a href="#"><img src={item} alt=""></img></a></li>
    )
  })
  return <ul> {Data} </ul>
}

const GerarCategorias =(props)=> {
  const Data = props.arr.map(({ link, img, legenda }) => {
    return (
      <>
        <li>
          <img src={img} alt=""></img>
...
const GerarCards =(props)=> {
  const Data = props.arr.map(({ titulo, texto, ong, local, candidatos, src }) => {
    return <Card titulo={titulo} texto={texto} ong={ong} local={local} candidatos={candidatos} src={src} />
  })
  return <CardsWrapper> {Data} </CardsWrapper>
}
